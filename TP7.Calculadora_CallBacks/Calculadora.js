...
btnIgual.addEventListener("click",() =>{
    if(igualHabiitado === true){
        ...
        if(numerosSignos.length === 1){impResultado.innerText = numeroIngresado; operacion.innerText = numeroIngresado;};
        if(numerosSignos.length === 3){calcular(); impResultado.innerText = nuemroResultado; operacion.innerText = nuemroResultado;};
        }
    }
    if(numeroNegativo === true){
        btnNeg.classList.toggle("colorAlert"); 
...
function presentarNumeros(){
    let element1 = "";
        if(numeroNegativo === true){
            for(let index = 0; index < numerosConcatenar.length; index++){
                const element = numerosConcatenar[index];
                operacion.innerText = "-" + element1 + element.toString();
                element1 = element1 + element.toString();
            }
        }
        if(numeroNegativo === false){
            for(let index = 0; index < numerosConcatenar.length; index++){
